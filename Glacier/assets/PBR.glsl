...
layout (std140, set = 0, binding = 1) uniform ObjectTransform
{
	mat4 u_Transform;
};
...
// Environment maps
#if VULKAN
layout (set = 1, binding = 4) uniform samplerCube u_EnvRadianceTex;
#else
layout(binding = 4) uniform samplerCube u_EnvRadianceTex;
#endif

#if VULKAN
layout (set = 1, binding = 5) uniform samplerCube u_EnvIrradianceTex;
#else
