...
namespace Snow {
	namespace Script {
		...
		static MonoMethod* GetMethod(MonoImage* image, const std::string& methodDesc) {
			...
			if (!desc)
				SNOW_CORE_ERROR("mono_method_desc_new failed");

			MonoMethod* method = mono_method_desc_search_in_image(desc, image);
			if (!method)
				SNOW_CORE_ERROR("mono_method_desc_search_in_image failed");
		...
		void ScriptEngine::LoadSnowRuntimeAssembly(const std::string& path) {
			MonoDomain* domain = nullptr;
			bool cleanup = false;
			if (s_MonoDomain) {
				domain = mono_domain_create_appdomain("Snow Runtime", nullptr);
				mono_domain_set(domain, false);

				cleanup = true;
			}

			s_CoreAssembly = LoadAssembly("assets/scripts/SnowScript.dll");
			...
			if (s_EntityInstanceMap.size()) {
				...
				if (s_EntityInstanceMap.find(scene->GetUUID()) != s_EntityInstanceMap.end()) {
					...
					for (auto& [entityID, entityInstanceData] : entityMap) {
						...
						SNOW_CORE_ASSERT(entityMap.find(entityID) != entityMap.end(), "Incalid entity ID or entity does not exist in scene!");
		...
		void ScriptEngine::Init(const std::string& assemblyPath) {
			...
			LoadSnowRuntimeAssembly(s_AssemblyPath);
		}
...

			{
				...
				while ((iter = mono_class_get_fields(scriptClass.Class, &ptr)) != NULL) {
					...
					if (oldFields.find(name) != oldFields.end())
						...
						PublicField field = { name, snowFieldType };
		...
		void ScriptEngine::InstantiateEntityClass(Entity entity) {
			...
			EntityInstanceData& entityInstanceData = GetEntityInstanceData(entity.GetSceneUUID(), id);
			EntityInstance& entityInstance = entityInstanceData.Instance;
		...
		void PublicField::CopyStoredValueToRuntime() {
			SNOW_CORE_ASSERT(m_EntityInstance->GetInstance());
