...
namespace Snow {
		...
		for (auto& shader : m_Specification.Shaders) {
			...
			if (shader->GetType() == Render::ShaderType::Vertex) {
				...
				for (spirv_cross::Resource& resource : res.stage_inputs) {
					...
					attribElementIndex++;
					size += Render::AttribElement::GetTypeSize(element.Type);
				}
			...
			for (spirv_cross::Resource& resource : res.uniform_buffers) {
				...
				if (m_UniformBuffers.find(bindingPoint) == m_UniformBuffers.end()) {
					...
					buffer.Buffer.BindingPoint = bindingPoint;
					buffer.Buffer.Size = size;
					buffer.Buffer.Name = name;
					buffer.Buffer.Stage = shader->GetType();
				}
	...
	VkShaderStageFlagBits VulkanPipeline::SnowShaderTypeToVKShaderStage(Render::ShaderType type) {
		switch (type) {
	...
	void VulkanPipeline::AllocateUniformBuffer(VulkanUniformBuffer& buffer) {
		...
		bufferCreateInfo.size = buffer.Buffer.Size;
		bufferCreateInfo.usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;

		VulkanAllocator allocator("UniformBuffer " + buffer.Buffer.Name);

		vkCreateBuffer(vkDevice->GetVulkanDevice(), &bufferCreateInfo, nullptr, &buffer.VKBuffer);

		VkMemoryRequirements memReqs;
		vkGetBufferMemoryRequirements(vkDevice->GetVulkanDevice(), buffer.VKBuffer, &memReqs);
		allocInfo.allocationSize = memReqs.size;

		allocator.Allocate(memReqs, &buffer.Memory, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
		vkBindBufferMemory(vkDevice->GetVulkanDevice(), buffer.VKBuffer, buffer.Memory, 0);

		buffer.DescriptorInfo.buffer = buffer.VKBuffer;
		buffer.DescriptorInfo.offset = 0;
		buffer.DescriptorInfo.range = buffer.Buffer.Size;
	}
